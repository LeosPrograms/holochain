...
pub struct NodeId(holo_hash::AgentPubKey);

impl Default for NodeId {
    fn default() -> Self {
        Self(holo_hash::AgentPubKey::from_raw_39(vec![0; 39]).unwrap())
    }
}

impl openraft::NodeId for NodeId {}
        ...
        I: IntoIterator<Item = Entry<TypeConfig>> + OptionalSend,
    {
        ...
        for entry in entries {
            let s = serde_json::to_string(&entry)
                .map_err(|e| StorageError::write_log_entry(entry.log_id(), &e))?;
...
impl RaftStateMachine<TypeConfig> for Arc<MemStateMachine> {
    ...
    async fn apply<I>(
        &mut self,
        entries: I,
    ) -> Result<Vec<ClientResponse>, StorageError<TypeConfig>>
    where
        I: IntoIterator<Item = Entry<TypeConfig>> + OptionalSend,
